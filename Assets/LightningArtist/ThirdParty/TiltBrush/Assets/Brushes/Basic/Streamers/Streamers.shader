...
Properties {
	...
	_TimeScaleY("Time Y", Float) = 1
	_TimeScaleZ("Time Z", Float) = 1
	...
	SubShader {
		Pass { 
			...
			fixed4 frag (v2f i) : COLOR {
				...
				half2 sins = sin(uvs.x * half2(10,23) + (_Time.z*_TimeScaleZ) * half2(5,3));
				...
				// Scrolling UVs
				uvs.x *= .5 + row_rand * .3;
				uvs.x -= (_Time.y * _TimeScaleY) * (1 + fmod(row_id * 1.61803398875, 1) - 0.5);
#endif

				// Sample final texture
				half4 tex = tex2D(_MainTex, uvs) * _Color;
